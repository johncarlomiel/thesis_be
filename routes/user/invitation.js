...
router.get('/invitation', verifyToken, (req, res) => {
   pool.query('SELECT DISTINCT invitation.event_id,invitation.user_id, users.* FROM invitation INNER JOIN events ON invitation.event_id = events.event_id WHERE user_id = ? AND status = ?', [req.userData.id, req.query.status], (err, results) => {
      if (err) throw err;
      res.json(results);
      console.log(results);
